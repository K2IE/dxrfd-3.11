...
/* K2IE - needed for check on dupe process */
#include <dirent.h>
#include <vector>
#include <iostream>
#include <fstream>
...
static void check_heartbeat()
{
   ...
   blocks_type::iterator block_pos;
   char dropped = ' ';
   bool rptr_active = false;

   char hb_buf[10];
   short int i,j;

   memcpy(hb_buf, OWNER, 8);
   hb_buf[9] = '\0';
   ...
   for (pos = a_user_list.begin(); pos != a_user_list.end(); pos++) 
   {
      ...
      if (block_pos == blocks.end())  /* not blocked */
      {
         // assume this repeater is not active
         rptr_active = false;

         // for each local reflector module 
         for (i = 0; i < 5; i++)
         {
            if (a_user_ptr->countdown[i] >= 0)
               a_user_ptr->countdown[i] --;

            if (a_user_ptr->countdown[i] < 0) 
            {
               // bands of this repeater, linked to this ref mod, are now dead
               a_user_ptr->rpt_mods[i][0] = a_user_ptr->rpt_mods[i][1] = 
               a_user_ptr->rpt_mods[i][2] = a_user_ptr->rpt_mods[i][3] = '\0';
            }
            else
            {
               rptr_active = true;

               // for each remote repeater band
               for (j = 0; j < 4; j++)
               {
                  // is there a link ?
                  ...
                  {
                     hb_buf[7] = 'A' + i;
                     hb_buf[8] = a_user_ptr->rpt_mods[i][j];
            
                     sendto(srv_sock, hb_buf, 10, 0,
                           (struct sockaddr *)&(a_user_ptr->sin),
                           sizeof(struct sockaddr_in));
                  }
               }
            }
...
static void print_users()
{
   ...
   for (pos = a_user_list.begin(); pos != a_user_list.end(); pos++)
   {
      ...
      snprintf(buf, 255, "%s,%s,REPEATER,%02d%02d%02d,%02d:%02d:%02d\n", 
               a_user_ptr->call,
               pos->first.c_str(),
               tm.tm_mon+1,tm.tm_mday,tm.tm_year % 100,
               tm.tm_hour,tm.tm_min,tm.tm_sec);
   ...
   for (pos2 = inbound_list.begin(); pos2 != inbound_list.end(); pos2++)
   {
      ...
      snprintf(buf, 255, "%s,%s,%s(%.8s),%02d%02d%02d,%02d:%02d:%02d\n",
               inbound_ptr->call,
               pos2->first.c_str(),
               ...
               for (user_pos = a_user_list.begin(), i_idx = 0; user_pos != a_user_list.end();  user_pos++, i_idx++)
               {
                  ...
                  { 
                     for (k = 0; k < 4; k++)
                     {
                        ...
                        {
                           refbuf[8 + (20 * j_idx)] = 'A' + module_idx;
...
/***************************************************************************************************/
      ...
      if (FD_ISSET(srv_sock, &fdset))
      {
         ...
         if ((recvlen == (CALL_SIZE + 2)) || (recvlen == (CALL_SIZE + 3)))
         {
            readBuffer[(recvlen - 1)] = '\0';

            // heartbeat ?
            if (recvlen == (CALL_SIZE + 2))
            {
               memcpy(a_call, (char *)readBuffer, 7);
               a_call[7] = ' ';
               a_call[8] = '\0';
            }
            else
            ...
            if (user_pos != a_user_list.end())
            {
               ...
               if (recvlen == (CALL_SIZE + 2))  // hb
               {
                  ...
                  if ((i >= 0) && (i <= 4) &&
                      (k >= 0) && (k <= 3))
                  {
                     a_user_ptr->countdown[i] = TIMEOUT;
                     for (j = 0; j < 5; j++)
                     {
                        if (i == j)
                           a_user_ptr->rpt_mods[i][k] = readBuffer[7];
                        else
               ...
               if (recvlen == (CALL_SIZE + 3))  // link or unlink 
               {
                  // link request:   KJ4NHF__BA0
                  // unlink request: KJ4NHF__B_0
                      ...
                      (readBuffer[8] == 'D'))
                  {
                     ...
                     if (readBuffer[9] == ' ')
                     {
                        ...
                        /* this is the remote repeater band */
                        k = readBuffer[8] - 'A';
                        ...
                        for (i = 0; i < 5; i++)
                        {
                           ...
                           {
                              traceit("Call %s mod %c drops the link to ref mod %c\n", a_call, readBuffer[8], 'A' + i);
                        ...
                        if (i == 5)
                        {
                           ...
                           */
                           // traceit("All linked modules from %s, %s have been unlinked, disconnecting...\n", a_call, an_ip);
                            ...
                            (readBuffer[9] == 'D') || (readBuffer[9] == 'E'))
                        {
                           ...
                           for (j = 0; j < 5; j++)
                           {
                              if (i == j)
                              {
                                 ...
                                 {
                                    traceit("Link established from %s mod %c ---> ref mod %c\n",
            ...
            else
            {
               ...
               // receiving a keepalive?, convert keepalive to link request
               if (recvlen == (CALL_SIZE + 2))
               {
                  recvlen = CALL_SIZE + 3;
                  memcpy(tmp_buf, readBuffer + 7, 3);
                  readBuffer[7] = ' ';
                  memcpy(readBuffer + 8, tmp_buf, 3);
                  readBuffer[10] = '\0';
                  traceit("converted keepalive to link request: [%s]\n", readBuffer);
               }
               ...
               if (allowed_to_connect)
               {
                  ...
                  if (a_user_ptr)
                  {
                     ...
                     {
                        traceit("new CALL=%s,REPEATER,ip=%s,users=%d, link from mod %c ---> ref mod %c\n",
                                a_call,
                                an_ip,
             ...
             (recvlen == 27))
         {
            ...
            if (found)
            {
               ...
               if (recvlen == 56)
               {
                  ...
                  if (readBuffer[25] == 'E')
                  {
                     a_user_ptr->countdown[4] = TIMEOUT;
               ...
               // What follows is NOT for recording
               if (rcd_list.find(an_rcd_streamid) == rcd_list.end())
               {
                  ...
                  if (inbound_list.size() > 0)
                  {
                     for (inbound_pos2 = inbound_list.begin(); inbound_pos2 != inbound_list.end(); inbound_pos2++)
                     {
                        ...
                        else
                        {
                           ...
                           {
                              /* treat the remote reflector with respect */
                              /* send it only if the module matches */
                              for (i = 0; i < 4; i++)
                              {
                                 if ((refbuf[14] == temp_r[i].hdr[14]) &&
...
/****************************** REPLACEMENT to send_a_pkt_to_all ****************************************************/
                  if (recvlen == 56)
                  {
                     ...
                     if (k >= 0)
                     {
                        a_user_ptr->countdown[k] = TIMEOUT;
                  ...
                  else
                  {
                     ...
                     if (i < 4)
                     {
                        for (user_pos2 = a_user_list.begin(); user_pos2 != a_user_list.end(); user_pos2++)
                        {
                           ...
                           if (fromUser.sin_addr.s_addr != a_user_ptr2->sin.sin_addr.s_addr)
                           {
                              if ((a_user_ptr2->rpt_mods[i][0] != '\0') || /* module is linked */
                                  (a_user_ptr2->rpt_mods[i][1] != '\0') ||
                                  (a_user_ptr2->rpt_mods[i][2] != '\0') ||
                                  (a_user_ptr2->rpt_mods[i][3] != '\0'))
                              {
                                 /* re-SYNC, source must NOT be a reflector */
                                 if (!(a_user_ptr->is_xrf))
                                 {
...
// From http://proswdev.blogspot.com/2012/02/get-process-id-by-name-in-linux-using-c.html
{
    int pid = -1;
...
// Check to see if dxrfd is already running
   int pid = getProcIdByName(argv[1]);
   if (pid > 0)
   {
      printf("dxrfd is already running.  Exiting...\n");
      return 1;
   }
